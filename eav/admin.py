...
class BaseEntityAdmin(ModelAdmin):
    ...
    def render_change_form(self, request, context, **kwargs):
        ...
        if self.exclude is not None:
            ...
            exclude = []
        eav_fields = filter(lambda x: (x not in model_fields and x not in exclude), all_fields)

        if self.eav_fieldsets:
            fieldsets_eav = self.eav_fieldsets + ((getattr(self, 'eav_fieldset_name', _('fields')), {'classes': ('eav-django-field-area',), 'fields': tuple(eav_fields)}),)
            fieldsets = fieldsets_eav
        else:
            # or infer correct data from the form
            fieldsets = [(None, {'fields': filter(lambda x: (x not in exclude), all_fields)})]
        ...
        try:
            ...
            inline_media = []
        if hasattr(inline_media, "media"):
            inline_media = inline_media.media
